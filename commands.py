...
def command_afterstream():
    pass


def command_crossplay():
...
def executeCommands(parentInputs, msg):
    msgParts = msg["snippet"]["textMessageDetails"]["messageText"].split(" ")
    cmd = msgParts[0]
    if cmd in commandNames.keys():
        if len(msgParts) > 1:
            parentInputs["sendText"](commandNames[cmd](
            ), msgParts[1:])
        else:
            parentInputs["sendText"](commandNames[cmd]())
